...
!                                        avgdata_u, avgdata_v
      ...
      REAL, DIMENSION(1645,24)::cf,atemp,humid
      REAL, DIMENSION(1640)::Qriver
      REAL, DIMENSION(1640)::Eriver
      DOUBLE PRECISION::begin_hour,end_hour  ! used in interpolate_dt
      DOUBLE PRECISION::t_o, t_f, dt, time, h_i, del, dummy_time, density_test, del_p 
...
!!!!For surface fluxes (surface_flux.f90): 
      ...
      INTEGER::M, steps, time_step, index,xx, xx2,year, month, jmax_i, windyear, &     
               count, yy, yy2, gg, jmaxg, cloud_type, day, day_o, stable, &   
               water_type,t_start, t_end, neg_count, current_day, day_check, day_check2, &
               migrate,cruise_id, year_o!, month_o  !migrate = 1 if timestep has copepod migration, else 0
      REAL::D_test
      DOUBLE PRECISION::D, lambda, u_star, w_star,L_star, day_time, Io, h_Ekman  !depth
...
!!!!!!!variables from KPP used in shift_vector.f90
      ...
      INTEGER::cloud_day,stormday,ironday
