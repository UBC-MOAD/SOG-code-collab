PROGRAM SOG      
      ...
      READ (10,*)M, D, lambda , t_o, t_f, dt, day_o, year_o, month_o, windyear, stormday,cruise_id
...
!PRINT*,day,day_met,cf(day_met,1)/10.,'day,day_met,cf(day_met,j)/10.'
    ...
    Qinter = (day_time*Qriver(day_met)+(86400-day_time)*Qriver(day_met-1))/86400.
    Einter = (day_time*Eriver(day_met)+(86400-day_time)*Eriver(day_met-1))/86400.
...
171 format (2x,i3,2x,3(e13.5,2x))
         END DO
...
!            write (*,*) ii,yy,pbx(yy),yy-cz,density%new(ii),'  **3'
         ...
         do yy=1,M
            pbx(yy) = (pbx(yy)-sumpbx)*gorLx*grid%i_space(yy)+stress%u%new/(1025*M*grid%i_space(yy))
            pby(yy) = (pby(yy)-sumpby)*gorLy*grid%i_space(yy)+stress%v%new/(1025*M*grid%i_space(yy))+dS(yy)*uplume*factor*K%u%all(h_m%g)/h_m%new*20.
...
111 format (3x,f7.3,7(2x,f7.3),1(2x,i4),2(2x,f10.4))
      ...
      do isusan=0,M
        ...
        write (4,131) K%u%all(isusan),K%s%all(isusan),I_par(isusan),K%t%all(isusan)
131 format (13(x,f10.4))
      end do
      close (4)
      ...
      density%new(isusan)=density%new(isusan)-1000
        ...
        write (6,131) T%new(isusan),s%NEW(isusan),density%new(isusan),P%micro%new(isusan),N%O%new(isusan),N%H%new(isusan),Detritus(1)%D%new(isusan),Detritus(2)%D%new(isusan),Detritus(3)%D%new(isusan),f_ratio(isusan)
      end do
      close (6)
