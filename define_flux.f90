SUBROUTINE define_flux
      ...
      DO xx = 1, h%i
         ...
         ELSE
            ...
            w%b(xx) = g*(alph%i(xx)*w%t(xx)-beta%i(xx)*w%s(xx))  ! buoyancy flux by definition given t and s flux
            w%b_err(xx) = g*(alph%idiv(xx)*w%t(xx) -beta%idiv(xx)*w%s(xx)) ! buoyancy flux variation due to error in z
            ...
            Q_t(xx) = -w%t(xx)*density%new(xx)*Cp%i(xx)
         END IF
      END DO
