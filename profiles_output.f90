...
300 format("*RunDateTime: ", a/,                                         &
  ...
  subroutine write_std_profiles(str_run_Datetime, str_CTD_Datetime, &
       ...
       NO, NH, DIC, Oxy, Si, D_DON, D_PON, D_refr, D_bSi, Ku, Kt, Ks, U, V)
...
400       format(i4, 2(1x, i2), 1x, i3, 3(1x, i2), 1x, i5, 2x, f6.2, 2x, &
          ...
          call write_profiles_numbers(profiles, grid, T, S, rho,   &
               Pmicro, Pnano, Ppico, Z, NO, NH, DIC, Oxy, Si, D_DON, D_PON, &
               D_refr, D_bSi, Ku, Kt, Ks, I_par, U, V)
          close(profiles)          
...
400 format("! Profiles of Temperature, Salinity, Density, ",          &
         ...
         "! Silicon, Detritus (DON, PON, refractory N & ",           &
...
500 format("*FieldNames: depth, temperature, salinity, sigma-t, ",     &
         ...
         "pico phytoplankton, ",                                       &
         "micro zooplankton, nitrate, ammonium, ",                     &
         "dissolved inorganic carbon, dissolved oxygen, silicon, ",    &
         ...
         "uM N, uM N, uM N, uM C, uM O2, uM, uM N, uM N, uM N, uM, ",  &
         "m^2/s, m^2/s, m^2/s, W/m^2, m/s, m/s")
  end subroutine write_cmn_hdr_fields


  subroutine write_profiles_numbers(unit, grid, T, S, rho, Pmicro,     &
       Pnano, Ppico, Z, NO, NH, DIC, Oxy, Si, D_DON, D_PON, D_refr,    &
...
!--- END CHEMISTY INPUT DECLARATIONS
    ...
    write(unit, 600) grid%d_g(0), KtoC(T(0)), S(0), sigma_t, &
         Pmicro(0), Pnano(0), Ppico(0), Z(0),                &
         NO(0), NH(0), DIC(0), Oxy(0), Si(0),                &
    ...
    do i = 1, grid%M
       ...
       write(unit, 600) grid%d_g(i), KtoC(T(i)), S(i), sigma_t,   &
            ...
            DIC(i), Oxy(i), Si(i), D_DON(i), D_PON(i), D_refr(i), &
