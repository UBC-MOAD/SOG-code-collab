...
class Test_check_diff(unittest.TestCase):
    ...
    def test_too_many_args(self):
        ...
        """
        argv = 'read_infile.py infile extra'.split()
        exception = self.assertRaises(SystemExit, read_infile, argv)
        self.assertEqual(exception.code, 1)
        self.assertEqual(sys.stdout.getvalue(), 
                         'usage: %(prog)s infile\n' % {'prog':argv[0]})
    ...
    def test_bad_file(self):
        ...
        """
        argv = 'read_infile.py badfile'.split()
        exception = self.assertRaises(SystemExit, read_infile, argv)
        self.assertEqual(exception.code, 1)
        self.assertEqual(sys.stdout.getvalue(), 
    ...
    def test_1_line_params(self):
        """print outfiles and exit with status=0 for 1 line param files.
        """
...
""")
        test_file.flush()
        argv = ['read_infile.py', test_file.name]
        exception = self.assertRaises(SystemExit, read_infile, argv)
...
profiles/nov04-test
profiles/hoff-nov04-test.dat
""")


    def test_multiline_params(self):
        """print outfiles and exit with status=0 for multiline param files.
        """
        test_file = tempfile.NamedTemporaryFile()
        test_file.write("""\
"dt"		900	 "time step [s]"
