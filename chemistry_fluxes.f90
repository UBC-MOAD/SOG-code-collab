...
contains
  ...
  subroutine solve_gas_flux(grid, dt, T, S, rho, unow, vnow, DIC, Oxy, &
       day, time)
    ...
    max_iter = 4
    do count = 1, max_iter !---- Begin Iteration Loop ----
...
!       call gas_flux('Oxy', T, S, Oxy(1), unow, vnow, Oxy_flux)
       ...
       ! vectors (*_RHS%sink).
       call build_chem_equations(grid, 225.0d0, DIC, DIC_flux, Oxy, Oxy_flux)
